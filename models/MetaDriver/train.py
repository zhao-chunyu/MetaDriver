...
from util.util import kldiv, sim, cc
...
def get_model(args):
    ...
    # Resume
    get_save_path(args)
    print('Model Save', args.snapshot_path)
...
def main():
    ...
    data_list = os.path.join(BASE_DIR, args.train_list)

    if args.data_type == 'metadada':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                     ...
                                     data_type=args.data_type, test_base=args.test_base, data_set=args.data_set, use_split_coco=args.use_split_coco)

    if args.data_type == 'metapsad':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                     base_data_root=args.base_data_root, data_list=data_list, \
                                     transform=train_transform, transform_tri=train_transform_tri, mode='train', \
                                     data_type=args.data_type, test_base=args.test_base, data_set=args.data_set, use_split_coco=args.use_split_coco)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # ----------------------  TRAIN  ----------------------
        loss_train = train(train_loader, val_loader, model, optimizer, epoch)

        train_loss_history.append(loss_train)
        np.save(args.snapshot_path + "/train_loss_history.npy", np.array(train_loss_history))


        # save model for <resuming>
        # train_final_filename = args.snapshot_path + '/HDMNet_epoch_{}.pth'.format(epoch-1)
        # if osp.exists(train_final_filename):
        #     os.remove(train_final_filename)
        train_final_filename = args.snapshot_path + '/HDMNet_epoch_{}.pth'.format(epoch)
        torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()},
                       train_final_filename)

        # -----------------------  VAL  -----------------------
        interval = 1
        if args.evaluate and epoch % interval == 0:

            loss_val = validate(val_loader, model, criterion, epoch)


            valid_loss_history.append(loss_val)
            np.save(args.snapshot_path + "/valid_loss_history.npy", np.array(valid_loss_history))
            ...
            if loss_val < min_loss_val:
                ...
                val_best_filename = args.snapshot_path + '/HDMNet_best.pth'
                logger.info('Saving checkpoint to: ' + val_best_filename)
                torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()},
                           val_best_filename)
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1} | train", unit="batch") as pbar:
        for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                metrics[0] += cc(output, target)
                metrics[1] += sim(output, target)
                metrics[2] += kldiv(output, target)

                #
                pbar.set_postfix(
                    loss=f"{loss_meter.avg:.4f}",
                    CC=f"{metrics[0].item() / (i + 1):.4f}",
                    SIM=f"{metrics[1].item() / (i + 1):.4f}",
                    KLD=f"{metrics[2].item() / (i + 1):.4f}"
                )
                pbar.update(1)
...
def train_ori(train_loader, val_loader, model, optimizer, epoch):
    global best_miou, best_FBiou, best_piou, best_epoch, keep_epoch, val_num
    global best_miou_m, best_miou_b, best_FBiou_m
    batch_time = AverageMeter()
    data_time = AverageMeter()
...
def validate(valid_loader, model, criterion, epoch):
    ...
    with torch.no_grad():
        with tqdm(total=len(valid_loader), desc=f"Epoch {epoch+1} | valid", unit="batch") as pbar:
...
def validate_ori(val_loader, model):
    if main_process():
